...
def validate_request(request: Request) -> Request:
    ...
    if (
        request.path
        and len(request.path.split("/")) == 2
        and not request.path in RESTRICTED_PATH_NAMES
    ):
    ...
    if (
        ...
        and paths[request.path].get("allowed_methods") is not None
    ):
    ...
    if not request.method in GLOBAL_METHODS:
        raise RequestException(
            f"Method is not allowed for path {request.path}",
            STATUS.METHOD_NOT_ALLOWED_405,
        )
